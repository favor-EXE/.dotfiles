...
awful.screen.connect_for_each_screen(function(s)
	...
	awful.tag({ "", "", "󰙯", "", "", "󰇮", "7", "8", "9" }, s, awful.layout.layouts[1])
...
-- Cheat sheet
	...
	end, { description = "toggle polybar's visibility", group = "polybar" }),
	...
	end, { description = "toggle titlebar visibility", group = "awesome" }),

	-- Launch terminal
	awful.key({ superkey }, "Return", function()
		awful.spawn(terminal)
	end, { description = "Launch terminal emulator", group = "applications" }),
	...
	end, { description = "Applications keychord", group = "applications" }),

	-- Rofi
	awful.key({ superkey }, "d", function()
		awful.spawn.with_shell("polybar-msg cmd show ; rofi -show drun")
	end, { description = "Rofi launcher", group = "applications" }),
	awful.key({ altkey }, "Tab", function()
		awful.spawn.with_shell("polybar-msg cmd show ; rofi -show window")
	end, { description = "Rofi Alt Tab Menu", group = "applications" }),

	-- Rofi keychord
	awful.key({ superkey, "Shift" }, "d", function()
		local grabber
		grabber = awful.keygrabber.run(function(_, key, event)
			if event == "release" then
				return
			...
			if key == "a" then
				awful.spawn.with_shell("polybar-msg cmd show ; rofi -show drun")
			elseif key == "r" then
				awful.spawn.with_shell("polybar-msg cmd show ; rofi -show run")
			elseif key == "p" then
				awful.spawn.with_shell("polybar-msg cmd show ; sh ~/.config/rofi/scripts/rofi-exit.sh")
			elseif key == "w" then
				awful.spawn.with_shell("polybar-msg cmd show ; rofi -show window")
			elseif key == "f" then
				awful.spawn.with_shell("polybar-msg cmd show ; rofi -show filebrowser")
			elseif key == "s" then
				awful.spawn.with_shell("polybar-msg cmd show ; rofi -show ssh")
			end
			awful.keygrabber.stop(grabber)
		end)
	end, { description = "Rofi scripts keychord", group = "applications" }),
	...
	end, { description = "increase slave height factor", group = "layout" }),
	awful.key({ superkey, "Shift" }, "k", function()
		awful.client.incwfact(-0.1)
	end, { description = "decrease slave height factor", group = "layout" }),
	...
	end, { description = "Switch to master and stack layout (master on the left)", group = "layout" }),
	awful.key({ superkey, "Control" }, "j", function()
		awful.layout.set(awful.layout.suit.tile.top)
	end, { description = "Switch to master and stack layout (master on the bottom)", group = "layout" }),
	awful.key({ superkey, "Control" }, "k", function()
		awful.layout.set(awful.layout.suit.tile.bottom)
	end, { description = "Switch to master and stack layout (master on the top)", group = "layout" }),
	awful.key({ superkey, "Control" }, "l", function()
		awful.layout.set(awful.layout.suit.tile.left)
	end, { description = "Switch to master and stack layout (master on the right)", group = "layout" }),
	...
	awful.key({ superkey, "Shift" }, "r", awesome.restart, { description = "reload awesome", group = "awesome" }),
	awful.key({ superkey, "Shift" }, "Escape", awesome.quit, { description = "quit awesome", group = "awesome" }),
...
-- Toggle window states
	...
	awful.key({ superkey }, "w", awful.client.floating.toggle, { description = "toggle floating", group = "client" }),
...
awful.rules.rules = {
	...
	{ rule_any = { type = { "normal", "dialog" } }, properties = { titlebars_enabled = show_titlebars } },
...
awful.spawn.with_shell("sh ~/.config/polybar/favor/launch.sh")
